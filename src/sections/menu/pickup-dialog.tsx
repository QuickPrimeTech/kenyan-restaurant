...
import {
  ...
  generateTimeSlots,
} from "@/utils/pickup-slot-generator";
...
export const PickupDialog = () => {
  ...
  const TIME_SLOTS = selectedDate ? generateTimeSlots(selectedDate, 15) : null;
                  ...
                  <div className="bg-muted/30 rounded-2xl overflow-hidden">
                    {!TIME_SLOTS?.length && (
                      <p className="p-4 text-sm text-gray-500">
                        No available time slots for the{" "}
                        {formatPickupDate(defaultDate || "")}.
                      </p>
                    )}
                    <RadioGroup
                      value={field.value ?? ""}
